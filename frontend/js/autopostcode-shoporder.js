(function($) {
    ...
    function add_find_btn(prefix){
        var brnhtml =
        jQuery('<p class="form-row form-row-first address-field autopostcode_' + prefix + '" ' + ' id="' + prefix + '_postcode_search_field" style="position:relative;">' +
            //'<label for="' + prefix + '_postcode_search">' + 'Postcode Search' + '</label>' +
            '<input type="text" ' + 'class="input-text autosearchinput" ' + 'name="' + prefix + '_postcode_search" ' + 'id="' + prefix + '_postcode_search" ' + 'placeholder="Search for postcode"> </p>' +
            '<p class="form-row form-row-last address-field autopostcode_' + prefix + '"> <button class="button autofindbtn alt" ' + 'id="' + prefix + '_autobutton" ' + 'type="button"> Search Address </button> </p>'+
        ...
        if (jQuery('#' + prefix + '_address_1').length) {
            ...
            if (auto_wc_country == auto_country || typeof auto_wc_country == 'undefined')
            {
                if (!jQuery('#' + prefix + '_autobutton').length) {
                    ...
                    jQuery('#' + prefix + '_address_1').closest('p').before(jQuery('.'+prefix+'_country_field'));
                    jQuery('.'+prefix+'_country_field').css({width: '100%'});
    ...
    if(auto_settings.postcode.active){
        ...
        auto_address_form('_billing');
        auto_address_form('_shipping');

        jQuery('#_billing_country').on('change', function(event) {
            auto_address_form('_billing');
        });
        jQuery('#_shipping_country').on('change', function(event) {
            auto_address_form('_shipping');
        });
    }
